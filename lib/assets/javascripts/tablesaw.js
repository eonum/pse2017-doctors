...
;(function( win, $, undefined ){
	...
	ColumnToggle.prototype.init = function() {
		...
		$btnContain.appendTo( this.$table.find('*').filter( '.' + this.classes.toolbar ) );
	...
	function createSwipeTable( $table ){
		...
		function advance( fwd ){
			...
			if( canAdvance(fwd) ){
				...
				$table.trigger( 'tablesawcolumns' );
				$table.trigger( 'tablesawcolumns.advance' );
			}
		}
			...
			_init: function(){
				...
				var addClassToTable = function(){
					...
					addSwitcher = function( heads ){
						...
						var $toolbar = el.find('*').filter( '.tablesaw-bar' ),
...
// adds the fixed table header by floatThead directly to the table
// to avoid and solve compatibility issues
;(function integratingFixedHeader( win, $, undefined ){


	function initFixedHeader( $table ){
		$table.floatThead();
	}

	// on tablesawcreate, initialize the fixedHeader
	// (not before or we will get problems)
	$( document ).on( "tablesawcreate", function( e, Tablesaw ){
			initFixedHeader( Tablesaw.$table );
	} );
	...
	var S = {
		...
		init: function( table ) {
			var $table = $( table ),
				...
				$toolbar = $table.find('*').filter( '.tablesaw-bar' ),
				modeVal = '',

...
(function( win, $, undefined ){
	...
	function SwipeTable( $table ) {
		var $btns = $("<div class='tablesaw-advance'></div>"),
			...
			hideBtn = 'disabled',
			tableHeader = $table.find( "thead th"),
			headerCells = tableHeader.not(".tablesaw-ignore"),
			headerCellsNoPersist = headerCells.not( '[data-tablesaw-priority="persist"]' ),
			$toolbarFiller = $(tableHeader.filter(".tablesaw-fill-cell")),
			$toolbarToolCell = $(tableHeader.filter(".tablesaw-tool-cell")),
		...
		/**
		 * Hides the toolbar if all columns are shown or shows it otherwise
		 */
		function adaptToolbarVisibility(){
			...
			if(toolbarVisible && allColumnsShown) {
				...
				$table.trigger( 'tablesaw-all-visible' );
			}
		...
		function advance( forward ){
			var shownHiddenPairs = changingColumnPairs( forward );

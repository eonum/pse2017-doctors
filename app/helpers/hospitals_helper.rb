module HospitalsHelper
  def variable_source variable, comparison
    ...
    ['kzp', 'qip', 'anq', 'bag_drgsearch'].each do |set|
  ...
  def hop_format_hospital_variable variable, comparison
    ...
    if(variable.variable_type == :relevance)
      return '' if value.blank?
      value = value.to_f
      return raw "<div class='meter relevance'><span style='width: 0%' id='numcase-#{variable.field_name}'></span></div><div class='numcase_overlay'>#{'%.1f' % value}%</div>"
    end
    value
  end
